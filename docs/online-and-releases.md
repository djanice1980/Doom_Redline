...
Written 2026-09-15 as an assessment before any of this work started. **It is
all built now.** Sections 1 to 9 are that original assessment, left as they
were written, with a note where what shipped went a different way. Section 10
is the current state and is the part kept up to date.

For how any of it works today rather than how it was planned:
`web/README.md` (the service and its setup), `docs/packaging.md` (how a
release is built), `docs/design.md` (the game), `CHANGELOG.md` (what changed
when).
...
> **What shipped instead.** There are no GitHub Actions workflows in the repo.
> `packaging/build-release.sh` builds all six artefacts on this machine and
> `gh release create` uploads them: the tarball, the `.deb`, the Arch package
> and the AppImage from `packaging/linux/container-build.sh` in a stock
> `archlinux:base-devel` container, the Windows zip from `fedora:42` with
> mingw-w64, and the installer from the `amake/innosetup` image under Wine.
> The Linux half has to be built in a container rather than on the host: a
> binary linked on CachyOS carries that distribution's microarchitecture level
> in its ELF notes and the loader refuses it on an ordinary CPU, which is what
> broke every Linux download in 0.7.0 (see `docs/packaging.md`). The version
> does come from `version.h` generated by CMake, as described; the tag follows
> the version rather than setting it.
...
> **What shipped instead.** An address is collected, because the approve link
> has to be sent somewhere. It is stored as AES-256-GCM ciphertext plus a
> one-way HMAC used for lookup, it is never shown to another player, and the
> only place it is ever decrypted is the owner's own admin page. Hardware
> identifiers are still not collected: the machine facts of section 6 are
> encrypted and keyed by the random install id. No profanity filter yet.
...
> Done, in this order, between 0.1.0 and 0.3.0. Kept for the record.
...
Written as notes, not a spec; nothing here was coded yet at the time.

> **What shipped instead.** All of it is built, with three deliberate changes
> made after playing it:
>
> * **No six-digit code.** 0.6.0 removed it. The email has one Approve button
>   and the game polls until the server says yes. Typing a code into a game
>   that is not a browser was more friction than the feature was worth.
> * **Collect first, show nothing.** Games are posted from the moment an
>   address is set, but no board, player page or total includes a player until
>   they have clicked Approve. Declining deletes everything collected for them.
>   The owner can therefore tell whether anyone is playing without anyone's
>   data being published.
> * **Permission belongs to the address, not to each profile.** A second
>   profile on a machine the address has already approved is approved with it;
>   a new machine is still asked. Deleting a profile takes its permission with
>   it.
>
> The tables are as sketched below except that there is no `sessions` table,
> and `player_machines`, `trophies`, `ratings`, `mail_log`, `rate_limits` and
> `settings` were added along the way.
...
## 10. Status (2026-09-21, 0.7.3)

**The game side.** Run records as JSON in `<profile>/runs`; ONLINE on by
default with a one-time question for players who have no registration;
registration under PLAYER EMAIL by one Approve button in the email, which the
game notices by polling; the update notice and the WHAT'S NEW screen fed by
`/api/version` from the GitHub release and `CHANGELOG.md`; uploads with a
pending queue; the LEADERBOARD screen with a cached copy; the players screen
(several people on one machine, rename, change address, delete, and the offer
to carry on as an existing player of the same address); a CONTROLS page and a
farewell page. `src/game/online.*`, `src/game/runrecord.*`, `src/net/http.*`
with libcurl on Linux and WinHTTP on Windows.

**The service** in `web/`: Next.js on Vercel, Postgres on Supabase through the
pooler with row-level security closing the REST API, an HMAC blind index and
AES-256-GCM for addresses, machine facts and the Graph credentials. Boards,
player pages, trophies, site totals and nightly ratings, all showing approved
players only. Mail through Microsoft Graph. `/admin` behind a password has two
tabs: the Graph settings with a mail log, and a dashboard of everything
collected including the players who have not approved yet, what people run the
game on, and the addresses decrypted for the owner. `web/README.md` is the
set-up guide; `npm run smoke` drives the whole flow against a local Postgres.

**Releases.** Six artefacts per tag, built by `packaging/build-release.sh` and
uploaded with `gh release create` (see the note under section 2 and
`docs/packaging.md`). The Linux ones are built in a stock Arch container so
they run on any x86-64 machine.

**Not built:** the replay verifier and the verified badge (section 5),
seasons, the profanity filter on display names, the OPTIONS > DATA screen of
section 6 (the machine facts ride along with each run instead, and the ONLINE
switch is the only toggle), and the `sessions` table.
