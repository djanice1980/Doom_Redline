...
export default async function ConfirmPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ t?: string }> }) {
  ...
  if (status === "declined") return <Done title="Declined" text="This registration was declined. Nothing is stored." />;
  if (status !== "pending" || expired) return <Bad text="This link has expired. Ask for a new one from the game (OPTIONS > PLAYER EMAIL)." />;
  return (
    <>
      <h1>Approve REDLINE registration?</h1>
      <p className="sub">Someone playing as <b>{reg.display_name as string}</b> on a machine called <b>{(reg.machine_label as string) || "unknown"}</b> wants to attach that player to <b>{masked}</b>.</p>
      <div className="card" style={{ maxWidth: 640 }}>
        <p>Approving means that, whenever that player has ONLINE switched on, each finished game is posted to the REDLINE leaderboard: the player name, the score and game statistics, and what the machine is (operating system, GPU, cores, memory, gamepad model).</p>
        <p>The address is stored encrypted, is never shown to other players, and can be removed on request. Each player and each machine that wants to use this address is asked separately.</p>
